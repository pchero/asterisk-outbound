...
static bool check_more_dl_list(struct ast_json* j_dlma, struct ast_json* j_plan)
{
	...
	ast_asprintf(&sql, "select * from `%s` where ("
			"(number_1 is not null and trycnt_1 < %ld)"
			" or (number_2 is not null and trycnt_2 < %ld)"
			" or (number_3 is not null and trycnt_3 < %ld)"
			" or (number_4 is not null and trycnt_4 < %ld)"
			" or (number_5 is not null and trycnt_5 < %ld)"
			" or (number_6 is not null and trycnt_6 < %ld)"
			" or (number_7 is not null and trycnt_7 < %ld)"
			" or (number_8 is not null and trycnt_8 < %ld)"
			")"
			" and res_dial != %d"
			";",
