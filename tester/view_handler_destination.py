# -*- coding: utf-8 -*-
"""
Created on Sat Dec 31 21:45:11 2016
...
class FrameMain(object):
    ...
    def frame_setup(self):
        self.frame_main()
        self.update_list_items("destination")
        return
    ...
    def frame_main(self):
        ...
        list_tree = ttk.Treeview(frame, columns=self.list_headers, show="headings", height=30)
        ...
        # Buttons
        bt_create = tk.Button(frame, text="Show", width=8, height=5, command=self._action_button_show)
        bt_create.grid(column=4, row=0, sticky=tk.E+tk.W+tk.N+tk.S)

        bt_create = tk.Button(frame, text="Create", width=8, height=5, command=self._action_button_create)
        bt_create.grid(column=4, row=1, sticky=tk.E+tk.W+tk.N+tk.S)

        bt_update = tk.Button(frame, text="Update", width=8, height=5, command=self._action_button_update)
        bt_update.grid(column=4, row=2, sticky=tk.E+tk.W+tk.N+tk.S)
        
        bt_delete = tk.Button(frame, text="Delete", width=8, height=5, command=self._action_button_delete)
    ...
    def _action_button_show(self):
        ...
        # get search uuid
        ret = tkSimpleDialog.askstring("Show destination", "Please enter a destination uuid")
        if ret == None:
            return
        ...
        if ret == "":
            ...
            data = {"Uuid":ret}
            self.action_handler.send_cmd_async("OutDestinationShow", data)
        return
    ...
    def _action_button_create(self):
        ...
        self.action_handler.send_cmd_async("OutDestinationCreate")
        return
    ...
    def _action_button_update(self):
        ...
        self.action_handler.send_cmd_async("OutDestinationUpdate", items)
        return
    ...
    def _action_button_delete(self):
        ...
        data = {"Uuid":uuid}
        self.action_handler.send_cmd_async("OutDestinationDelete", data)
        return
    ...
    def update_list_items(self, table):
        ...
        if table == None or table != "destination":
            return

        self.list_items = self.data_handler.destination_get_list_all()
    ...
    def update_detail_items(self, uuid):
        ...
        data = self.data_handler.destination_get(uuid)
        if data == None:
            print("Could not find correct destination info. uuid[%s]" % uuid)
            return
